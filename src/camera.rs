...
pub fn track_camera(
    ...
    if let Ok(player_global_transform) = player_query.get_single() {
        if let Ok((mut cam_transform, projection)) = camera_query.get_single_mut() {
            if let Ok((map_size, grid_size, map_global_transform)) = tilemap_query.get_single() {
                ...
                let cam_half_width = projection.area.width() / 2.0;
                let cam_half_height = projection.area.height() / 2.0;
